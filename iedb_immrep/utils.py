...
def create_leaders_constants(species):
    global leaders, constants
    leaders, constants = get_leader_constant_seqs(species)


def get_leader_constant_seqs(species="human"):
    leaders = {}
    constant = {}
    for chain in ["TRA", "TRB"]:
        imgt_dat, tcr_functionality, partial = fxn.get_ref_data(
            chain,
            ["LEADER", "CONSTANT", "VARIABLE", "JOINING"],
            species.upper(),
        )
        leaders[chain] = set()
        constant[chain] = set()
...
def get_cdrs(
    path: str = os.path.join(data_path, "dat/trv_cdrs.tsv"), species="human"
):
    if species != "human":
        return None
    ...
    if method == "IEDB":
        return set(
            exclude[
                ...
                & (exclude["Reference - IEDB IRI"].notna())
            ]["Reference - IEDB IRI"]
        )
    else:
        return set(
            exclude[(exclude["Exclude"] == True) & (exclude["PMID"].notna())][
                "PMID"
            ]
        ).union(
            set(
...
def summarize_counts(
    df,
    receptor_key: str = "Receptor - IEDB Receptor ID",
    epitope_key: str = "Epitope - Name",
):
    return {
        "total": df.shape[0],
        "receptors": df[receptor_key].nunique(),
        "epitopes": df[epitope_key].nunique(),
    }


def prioritize_vj(df, chain):
    v_call = df[f"{chain} - Calculated V Gene"].where(
        df[f"{chain} - Calculated V Gene"].notna(),
        df[f"{chain} - Curated V Gene"],
    )
    j_call = df[f"{chain} - Calculated J Gene"].where(
        df[f"{chain} - Calculated J Gene"].notna(),
        df[f"{chain} - Curated J Gene"],
    )
    cdr3 = df[f"{chain} - CDR3 Calculated"].where(
        df[f"{chain} - CDR3 Calculated"].notna(), df[f"{chain} - CDR3 Curated"]
    )
...
def assert_correct_chain_order(df):
    if "TRB" in str(df["Chain 1 - v_call"]):
        df["Chain 1 - v_call"], df["Chain 2 - v_call"] = (
            df["Chain 2 - v_call"],
            df["Chain 1 - v_call"],
        )
        df["Chain 1 - j_call"], df["Chain 2 - j_call"] = (
            df["Chain 2 - j_call"],
            df["Chain 1 - j_call"],
        )
        df["Chain 1 - junction"], df["Chain 2 - junction"] = (
            df["Chain 2 - junction"],
            df["Chain 1 - junction"],
        )
...
def run_thimble(
    df, chains=["A", "B"], missing_strategy="remove", species="human"
):
    outputs = {}
    for chain in chains:
        format_for_thimble(
            df,
            chain=chain,
            output=f"thimble{chain}.tsv",
            missing_strategy=missing_strategy,
        )
        full_length_trs, new_cdr3s = run_thimble_single_chain(
            f"thimble{chain}.tsv", chain=chain, species=species
        )
        full_length_trs = {
            p: remove_leader_constant(full_length_trs[p], "TR" + chain)
            for p in full_length_trs
        }
        os.remove(
            f"thimble{chain}.tsv",
        )
...
def run_thimble_single_chain(
    thimble_input_path, chain="A", retry=True, species="human"
):
    print(f"Running Thimble for chain {chain} with terminal Fs.")
    output_path = f"output_thimble.tsv"
    out_subprocess = subprocess.call(
        f"thimble -in {thimble_input_path} -o {output_path} -s {species}",
        shell=True,
    )
    if out_subprocess != 0:
    ...
    if retry:
        ...
        print(
            f"Running Thimble for chain {chain} on failures ({perc_failed}%) with terminal Ws."
        )
        retry_path = thimble_input_path.replace(".tsv", "_redo.tsv")
        input_file = pd.read_csv(thimble_input_path, sep="\t")
        to_redo = set(input_file["TCR_name"].unique()).difference(
            set(fixed["TCR_name"].unique())
        )
        to_redo_df = input_file[input_file["TCR_name"].isin(to_redo)].copy()
        to_redo_df[f"TR{chain}_CDR3"] = to_redo_df[f"TR{chain}_CDR3"].map(
            lambda x: x[:-1] + "W"
        )
        to_redo_df.to_csv(retry_path, sep="\t", index=False)
        subprocess.call(
            f"thimble -in {retry_path} -o {output_path} -s {species}",
            shell=True,
        )
        outputs = pd.read_csv(output_path, sep="\t")
        output.update(
            dict(
                outputs[outputs[f"TR{chain}_aa"].notna()][
                    ["TCR_name", f"TR{chain}_aa"]
                ].values
            )
        )
        new_cdr3bs = dict(
            outputs[outputs[f"TR{chain}_aa"].notna()][
                ["TCR_name", f"TR{chain}_CDR3"]
            ].values
        )
        os.remove(output_path)
...
def format_for_thimble(
    df_input, chain="A", output="thimble.tsv", missing_strategy="remove"
):
    df = df_input.copy()
    thimble_columns = [
        "TCR_name",
...
def filter_for_information(
    df, chains=["Chain 1", "Chain 2"], fields=["v_call", "j_call", "junction"]
):
    filter_chains = [
        f"{chain} - {field}" for chain, field in product(chains, fields)
    ]
...
def valid_mhcclassI(mro_path=None, species="human"):
    if mro_path is None:
        mro_path = f"dat/MRO_molecules_{species}.tsv"
        with open(mro_path, "w") as k:
            k.writelines(
                requests.get(
                    f"https://raw.githubusercontent.com/IEDB/MRO/refs/heads/master/ontology/molecule.tsv"
                ).content
            )
    mro = pd.read_csv(mro_path, sep="\t")
    mro = mro[mro["In Taxon"] == f"{species}"]
    allowed_alleles = set(
        mro[(mro["Parent"] == "MHC class I protein complex")][
            "IEDB Label"
        ].unique()
    )
...
def validate_file(df, cdr_keys=["CDRA1", "CDRA2", "CDRA3"], seq_key="TRA"):
    ...
    for key in cdr_keys:
        df[f"{key}_val"] = df.apply(
            lambda x: x[key] in x[seq_key] if pd.notna(x[seq_key]) else True,
            axis=1,
        )
    return df


def cdrs_from_ndm_file(
    ndm_path="dat/igblast/internal_data/human/human.ndm.imgt",
    seq_path="dat/igblast/fasta/imgt_human_tr_v.fasta",
    outf="dat/trv_cdrs.tsv",
):
    input = pd.DataFrame(
        [
            p.strip("\n").split()
            for p in open(ndm_path, "r").readlines()
            if p.startswith("TR")
        ]
    )
    input.columns = [
    ...
    seqs = dict(
        (p.id, str(p.seq)) for p in SeqIO.parse(seq_path, format="fasta")
    )
    cdrs = {}
    ...
    pd.DataFrame(cdrs).T.reset_index().rename(
