...
tcr_search_params = {
    "mhc_classes": "cs.{I}",
    "qualitative_measures": "cs.{Positive}",
    # "host_organism_names": "cs.{h}",
    "order": "receptor_group_id",
    "receptor_type": "eq.alphabeta",
    "offset": 0,
}
...
def rename_func(str_name):
    ...
    suffix = (
        " ".join([p.capitalize() for p in suffix.split(" ")])
        .replace("Cdr", "CDR")
        .replace("Iedb", "IEDB")
        .replace("Ids", "IDs")
        .replace("Id", "ID")
...
def api_to_export(api_table):
    api_table.columns = [
        rename_func(p) if "__" in p else p for p in api_table.columns
    ]
    return api_table
...
def read_tcr_search_table(tcr_search_params):
    results = []
    result = requests.get(
        os.path.join(base_uri, "tcr_search"), params=tcr_search_params
    )
...
def get_tcr_export():
    ...
    results_export = []
    result = requests.get(
        os.path.join(base_uri, "tcr_export"), params=tcr_export_params
    )
...
def read_epitope_table(structure_ids):
    ...
    for p in range(0, len(structure_ids), 100):
        ...
        epitope = pd.json_normalize(
            requests.get(
                os.path.join(base_uri, "epitope_search"), params=assay_search
            ).json()
        )
...
def filter_export(results_export, receptor_ids):
    results_export = results_export[
        results_export["receptor__iedb_receptor_id"].isin(receptor_ids)
    ]
    # criteria not valid for mice
    # results_export = results_export[
    #     results_export["assay__mhc_allele_names"].fillna("").str.contains(":")
    # ]
    results_export = results_export[
        ~results_export["assay__mhc_allele_names"].isnull()
    ]
    return results_export
...
def read_assay_table(assay_ids):
    ...
    for p in range(0, len(assay_ids), 100):
        ...
        epitope = pd.json_normalize(
            requests.get(
                os.path.join(base_uri, "tcell_search"), params=assay_search
            ).json()
        )
...
results = results[
    results["host_organism_names"].apply(
        lambda x: "Mus"
        in (" ".join(x) if isinstance(x, (list, tuple)) else str(x))
    )
]
...
structure_ids = list(
    set().union(
        *[set(p.split(", ")) for p in output["epitope__iedb_iri"].unique()]
    )
)
...
results_export["Epitope - Structure IRI"] = results_export[
    "Epitope - IEDB IRI"
].map(lambda x: "IEDB_EPITOPE:" + x.split("/")[-1])
results_export = pd.merge(
    left=results_export,
    right=epitope_table[["e_modification", "structure_iri", "structure_type"]],
    left_on="Epitope - Structure IRI",
    right_on="structure_iri",
)
results_export = results_export[
    (results_export["structure_type"] == "Linear peptide")
    & (results_export["e_modification"].map(str) == "None")
]

assay_infos = dict(assay_table[["tcell_id", "assay_names"]].values)
results_export["Assay - Names"] = results_export["Assay - IEDB IDs"].map(
    lambda x: ",".join(set([assay_infos[int(p)] for p in x.split(", ")]))
)
results_export["Epitope ID"] = (
    results_export["Epitope - Name"]
    + "~"
    + results_export["Assay - MHC Allele Names"].replace(", ", "+")
)
results_export["Epitope IDs"] = results_export.apply(
    lambda x: ",".join(
        [
            x["Epitope - Name"] + "~" + p
            for p in x["Assay - MHC Allele Names"].split(", ")
        ]
    ),
    axis=1,
)
results_export.to_parquet(f"dat/api_export_results_MOUSE_I.parquet")
