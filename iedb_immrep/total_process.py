...
class IEDBDataset:
    ...
    def filter_mhc(self):
        ...
        if self.species == "human":
            self.df = self.df[
                (
                    self.df["Assay - MHC Allele Names"]
                    .fillna("")
                    .str.contains(":")
                )
            ...
            self.df = self.df[
                (~self.df["Assay - MHC Allele Names"].str.contains("mutant"))
                & (~self.df["Assay - MHC Allele Names"].isna())
            ]
    ...
    def prioritize_entries(self, prioritize_func=prioritize_vj):
        ...
        paired = pd.merge(
            left=df, right=df_chain1, left_index=True, right_index=True
        )
        paired = pd.merge(
            left=paired, right=df_chain2, left_index=True, right_index=True
        ).fillna(value=np.nan)
        has_chain = filter_for_information(
            paired, chains=self.chains, fields=["v_call", "junction"]
        )
        self.has_chain = has_chain
        self.ids["VJCDR3 for Relevant chains"] = set(
            self.has_chain["Receptor - IEDB Receptor ID"]
        )
        self.counts["VJCDR3 for Relevant chains"] = summarize_counts(
            self.has_chain
        )

    def standardize_genes(self, correct_j=True):
        if self.species == "human":
            standardize_spec = "homosapiens"
        elif self.species == "mouse":
            standardize_spec = "musmusculus"

        has_chain = self.has_chain
        for chain in self.chains:
            has_chain[f"{chain} - v_call_tt"] = has_chain[
                f"{chain} - v_call"
            ].map(lambda x: tt.tr.standardize(x, species=standardize_spec))
            has_chain = has_chain[(has_chain[f"{chain} - v_call_tt"].notna())]
            if correct_j:
                has_chain[f"{chain} - j_call_tt"] = has_chain[
                    f"{chain} - j_call"
                ].map(
                    lambda x: (
                        tt.tr.standardize(x, species=standardize_spec)
                        if pd.notna(x)
                        else "unknown"
                    )
                )
                has_chain = has_chain[
                    (has_chain[f"{chain} - j_call_tt"].notna())
                ]
        for chain in self.chains:
            has_chain[f"{chain} - v_call_tt"] = has_chain[
                f"{chain} - v_call_tt"
            ].map(lambda x: x + default_allele if "*" not in x else x)
            if correct_j:
                has_chain[f"{chain} - j_call_tt"] = has_chain[
                    f"{chain} - j_call_tt"
                ].map(
        ...
        self.has_chain = has_chain
        self.ids["Standardized"] = set(
            self.has_chain["Receptor - IEDB Receptor ID"].unique()
        )
    ...
    def run_stitchr(self):
        ...
        if "Chain 1" in self.chains:
            self.all_cdrs["TRA"] = self.all_cdrs["TCR_name"].map(
                thimble_outputs["A"][0]
            )
        if "Chain 2" in self.chains:
            self.all_cdrs["TRB"] = self.all_cdrs["TCR_name"].map(
                thimble_outputs["B"][0]
            )
    ...
    def assign_cdrs(self):
        ...
        for chain in self.chains:
            if self.species == "human":
                has_chain[f"CDR{mappings[chain]}1"] = has_chain[
                    f"{chain} - v_call_tt"
                ].map(self.cdr1s)
                has_chain[f"CDR{mappings[chain]}2"] = has_chain[
                    f"{chain} - v_call_tt"
                ].map(self.cdr2s)
                r = range(1, 4)
            else:
            ...
            has_chain[f"CDR{mappings[chain]}3"] = has_chain[
                f"{chain} - junction"
            ].map(lambda x: x[1:-1])
            keys += [f"CDR{mappings[chain]}{i}" for i in r]
            all_cdrs = has_chain[has_chain[keys].notna().all(axis=1)]

        self.counts["Assigned CDRs"] = summarize_counts(all_cdrs)
        self.ids["Assigned CDRs"] = set(
            all_cdrs["Receptor - IEDB Receptor ID"].unique()
        )
    ...
    def format(
        ...
        rename_columns = {
            "Chain 1 - v_call_tt": "TRAV",
            "Chain 2 - v_call_tt": "TRBV",
            "Chain 1 - j_call_tt": "TRAJ",
            "Chain 2 - j_call_tt": "TRBJ",
        }
        ...
        if self.species == "human":
            ...
            cdr_keys_1 = ["CDRA3"] if "Chain 1" in self.chains else []
            cdr_keys_2 = ["CDRB3"] if "Chain 2" in self.chains else []
        cdr_keys = cdr_keys_1 + cdr_keys_2
        self.all_cdrs["unique_id"] = self.all_cdrs.apply(
            lambda x: "_".join([x[p] for p in cdr_keys + ["Epitope IDs"]]),
            axis=1,
        )
        self.all_cdrs = self.flag_10X_studies(
            self.all_cdrs, exclusion_path=exclusion_path
        )
    ...
    def flag_10X_studies(
        ...
        df["10X"] = (
            df["Reference - IEDB IRI"]
            .map(lambda x: x.replace("http", "https"))
            .isin(studies)
        )
        return df
    ...
    def write_parquet(self, output):
        ...
        pd.DataFrame(self.counts).to_csv(
            output.replace(".parquet", "_filtering.csv")
        )
    ...
    def validate(self, df):
        ...
        val_columns = []
        if "Chain 1" in self.chains:
            validated_file = validate_file(df, cdr_keys=cdra_keys)
            if self.species == "human":
    ...
    def format_for_competition(self, output_path="immrep_IEDB.csv"):
        ...
        for k, df in self.all_cdrs.groupby("unique_id"):
            for i, (x, p) in enumerate(df.groupby(f"full_seq_unique")):
                example = p.iloc[0]
                receptor_ids = ",".join(
                    [str(p) for p in set(p["Receptor - IEDB Receptor ID"])]
                )
        ...
        ].rename(columns=rename_columns).to_csv(output_path, index=False)
    ...
    def format_airr(self, output_path="immrep_IEDB_airr.tsv"):
        ...
        if self.species == "human":
            ...
            chain1_cols = ["CDRA3", "TRAV", "TRAJ", "TRA"]
            chain2_cols = ["CDRB3", "TRBV", "TRBJ", "TRB"]
        renaming = {
            p: p[:3].lower() + p[-1] + "_aa"
