...
parser.add_argument(
    "--export_file", dest="export_file", help="Parquet export file from IEDB."
)
parser.add_argument(
    "--output_dir", dest="output_dir", help="directory to put output"
)
parser.add_argument(
    "--chains", dest="chains", default="paired", choices=["paired", "beta"]
)
parser.add_argument(
    "--species", dest="species", default="human", choices=["human", "mouse"]
)
args = parser.parse_args()

# total hack
create_leaders_constants(args.species)
...
iedb = IEDBDataset(
    args.export_file, file_type="parquet", chains=chains, species=args.species
)
...
print(f"Writing.")
iedb.format_for_competition(
    output_path=os.path.join(args.output_dir, "immrep_IEDB.csv")
)
# iedb.format_airr(
#     output_path=os.path.join(args.output_dir, "immrep_IEDB_airr.tsv")
# )
